...
**33.5 MB 那一行只有 3.75 GB/s**，低于 §7.1 的单流只读锚点（11.7–12.2 GB/s）——超出 L3 之后
是什么在限制还没查清（候选：每行都要重读的 `x`、以及 L1 未命中延迟没被完全掩盖），
**这条先按实测记下来，不编原因**。**完整表在 `docs/dev.md` §7.4**。
...
- **实测**：`gemv` **3.75–48.2 GB/s**、`dot` **41.8–68.5 GB/s**（单线程；口径 = 权重字节
  ...
  （低于 `docs/dev.md` §7.1 的单流锚点），**原因未查清、按实测记录**。**完整表在 `docs/dev.md` §7.4**。
...
|---|---|---|---|
| f16 GEMV（decode） | **6.8 GB/s** | `lasx_gemv_f16` **3.75–48.2 GB/s**（同量级形状 `1024×2048` = 10.5 GB/s ≈ 1.5×） | 小形状在缓存里时领先很多；超出 L3 后回落到 3.75 GB/s（原因未查清，见 §5.13） |
| f16/bf16 GEMM（prefill） | 8.4 / 16.6 GFLOP/s | f32 GEMM 60–67 GFLOP/s（§7.3），**没有 f16 GEMM** | 仍是未做的缺口（N1 只做了 f16 的 GEMV/dot） |
