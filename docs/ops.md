...
> 归 `docs/dev.md`**；正文出现的读数只是**指针 + 一行结论**，权威表在 `docs/dev.md` §7
> （2026-09-25 全量复测）。这是那次重写定下的规矩：**同一份数字只存一处**。
>
...
- **下溢恰好落到 0**：`x < −126.5/log2e = −87.6831` 时 `kk = 0` ⇒ `from_bits(0) = +0.0`
  ...
  `[−104, +88]`（既保证这点，也挡住 magic 数取整失效与浮点转整型越界）；
...
逐元素、**无归约**，所以位精确几乎是白送的：没有"两种结合次序"，只要标量与向量跑同一串 op
就逐位相同。真正需要写死的是**公式的等价形式**。
...
- **契约里的关键一条：写直接除法 `x / den`，不是 `x · (1/den)`**。两者数学等价、末位可能不同；
  参考实现必须照抄直接除法才算逐位一致。（softmax 那边相反：它的分母被所有列共享，一次
  倒数换 N 次除法是划算的——这里是每元素各自一次，没有共享机会。）
...
让分母停在有限值。把这条写出来是为了让调用方不必猜：**别把 `±inf` 喂进激活**。
...
与 §2.7 的 rms_norm 同为 2 条流，吞吐却只有它的一半到三分之一 ⇒ **这些算子是算力受限，
不是带宽受限**（同样的访存口径下 rms_norm 17.99 GB/s vs `silu` 7.13 GB/s，n ≈ 1M、
工作集 8 MB 仍在 L3 内）。瓶颈是 `exp`：每 8 个元素约 19 条向量指令外加**一次向量除法**，
而 rms_norm 每 8 个元素只要 2 条 `fma`。想再快得换更短的 `exp` 多项式（精度换速度）或
多项式整体重排，不属于本轮。**完整表在 `docs/dev.md` §7.4**。
...
**不是带宽**（纯 f16 流能跑 15.8–16.4 GB/s），**也不是"`x` 被挤出 L1"**（多行微内核实测无收益），
而是 `f16→f32` 转换（+2.2 ms）与 `permi_q` 重排（+1.6 ms）占了这个 7.8 ms 的一半——两者的
数量由契约决定（`cvt` 每 8 元素一条是 ISA 上限；`permi_q` 是为了"元素下标 `j` 落在 lane `j%8`"
的自然序，去掉就破坏位精确）。所以这是**结构代价，不是缺陷**；想更快必须动 §2.11 的契约。
...
- **实测**：`gemv` **3.75–48.2 GB/s**、`dot` **41.8–68.5 GB/s**（单线程；口径 = 权重字节
  ...
  **瓶颈已查清是 `x` 被权重流挤出 L1**（探针见 `docs/dev.md` §20.7）。**完整表在 `docs/dev.md` §7.4**。
...
|---|---|---|---|
| f16 GEMV（decode） | **6.8 GB/s** | `lasx_gemv_f16` **3.75–48.2 GB/s**（同量级形状 `1024×2048` = 10.5 GB/s ≈ 1.5×） | 小形状在缓存里时领先很多；超出 L3 后回落到 3.75 GB/s，瓶颈是 `x` 被权重流挤出 L1（已查清，见 §5.13） |
