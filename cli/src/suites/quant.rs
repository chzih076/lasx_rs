...
pub fn dot_q4(rows: &mut Vec<Row>) {
    for &n in &[1usize << 16, 1 << 22] {
        let mut rng = Lcg::new(0x9a17 ^ n as u64);
        let qa: Vec<u8> = (0..n).map(|_| rng.u8()).collect();
        let qb: Vec<u8> = (0..n).map(|_| rng.u8()).collect();
        let groups = n.div_ceil(32);
        let sa: Vec<f32> = (0..groups)
            .map(|_| 0.01 + 0.001 * rng.f32().abs())
            .collect();
        let sb: Vec<f32> = (0..groups)
            .map(|_| 0.01 + 0.001 * rng.f32().abs())
            .collect();
        let lasx = time_mode(Mode::Lasx, || {
