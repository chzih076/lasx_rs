...
/// `1/n`，且 `n == 0` 的 lane **精确置 0**：先无条件算 `1/0 = +inf`，再用
/// `n == 0` 的掩码配合 `andn` 把那些 lane 的清零。
///
/// 为什么不用分支/比较混合：这条路径一分钱都不多花（除法反正要算），也不需要
/// "按 lane 选值"的第二条指令；而**必须**清零是因为 `0 · inf = NaN`——
/// 契约要求零向量单位化输出 `(0,0,0)`（`docs/ops.md` §2.4）。
#[inline]
