...
//!
//! **单个 8 lane 累加器**，每个 16 元素块里先低 8 个、后高 8 个：
//!
//! ```text
//! acc = fma(cvtl(h[i..i+16]), b[i..i+8],  acc)     // 元素 i+l   → lane l
//! acc = fma(cvth(h[i..i+16]), b[i+8..i+16], acc)   // 元素 i+8+l → lane l
//! ```
//!
//! 即**元素下标 `j` 落在 lane `j % 8`**；尾部（不足 16 个）走标量、**延续同一 lane**
//! （`t[j % 8] += …`，两步舍入），最后按**固定次序两两归约**（与 softmax/rms_norm 同一棵树）。
//!
...
mod tests {
    ...
    ///
    /// 这是本轮最值钱的一条测试：它把"转换精确"从"抽几个值看看"变成**全覆盖**，
