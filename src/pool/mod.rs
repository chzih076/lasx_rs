...
//! # 为什么放进库里
//!
//! 多步传播这类真实负载里，"每次调用都新建线程"的代价摊不掉：实测每步约 0.48 ms
//! 的创建/回收开销，而多星多步传播端到端因此慢 **2.55×**（477 µs vs 187 µs；
//! 2026-09-25 复测，完整表在 `docs/dev.md` §7.7）。常驻池把这个开销降到接近零，
//! 并让**建池只发生一次**。
//!
//! # 三种切分形状
//!
...
//! 倍数，好处是块边界与内核的微块边界对齐（`lasx_matmul` 一次 4 行、B 复用 4 次）。
//! **注意本文件早期版本在这里写过"`row_gran=1` 慢一倍"——那条 2026-09-25 已被复测推翻**
//! （gran=1 从没更慢，256³/12 线程上还快 8%，512³/1024³ 持平；见 `docs/dev.md` §14 开头）。
//!
//! ```
...
//!
//! 第一条（"只读数组只能读本块"）是这套接口唯一"靠约定而不是靠编译器"的地方，所以
//! `start` 必须传出来：只读数组可以整个捕获进闭包，**按 `start` 切片**即可，不需要
//! `unsafe`、也不需要"把只读入参假装成 `&mut`"。上层（`shape::Prepared`）生成这个闭包
//! 时必须用 `start` 切片。
//!
//! 第三条：**输入与输出不能是同一块内存**（原地 `y = a·x + y` 那种累加）。
//!
//! - 在**用户 API** 下不可达：`Prepared::apply(&x, &mut y)` 里 `x`/`y` 是两个不同绑定；
//!   若想用同一个绑定，`let xv = &x; apply(xv, &mut x)` 会被借用检查器直接拒绝
//!   （`E0502`——它按**路径**而不是按类型判断，所以拦得住；本条是实测确认的）。
//! - 在**库内生成代码 / `unsafe`** 下仍可能：从裸指针造出两个切片，就能构造出"同一块
//!   内存，一个 `&`、一个 `&mut`"，类型系统看不到。
//!
//! 所以结论不是"已经不可能"，而是"用户触发不了、库作者可能触发"——`shape::Prepared`
//! 因此在入口加了 `debug_assert!(输入指针 != 输出指针)`，宏生成代码时必须保证
//! 输入和输出是**不同的变量**。
//!
...
impl WorkerPool {
    ...
    /// 批量内核没有额外收益，也不会更差（`docs/dev.md` §3.1 与 §7.7 的线程扫描）。
    pub fn auto() -> Self {
        Self::new(
